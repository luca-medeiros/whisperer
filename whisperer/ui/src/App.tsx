...
function DreamSearch() {
  ...
  const dreamIt = async () => {
    if (query && lightningState && !loading) {
      ...
      setLoading(true);
      try {
        const result = await postDream(query, highQuality, lightningState.vars.whisperer_url);
  ...
  return (
    <div>
      {!lightningState?.vars?.whisperer_url && <OverlayLoader />}
            ...
            <Container maxWidth={'sm'} disableGutters>
              ...
              <Row sx={{ 'justifyContent': 'space-between', '>div': { width: '100%' } }}>
                ...
                <Box sx={{ '.MuiButton-root': { borderRadius: 40 } }}>
                  <Button
                    disabled={!query || loading || !lightningState?.vars?.whisperer_url}
                    text="Muse"
                    onClick={dreamIt}
                    fullWidth
                    icon={<FlashesIcon />}
